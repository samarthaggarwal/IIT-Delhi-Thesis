\section{Overview}
  ...
  Open Information Extraction (Open IE) refers to the task of forming relational tuples from sentences, without a fixed relation vocabulary \cite{TextRunner}. Table \ref{tab:openie_eg} enlists a few examples of sentences along with their OpenIE tuples.
...
\end{verbatim}
  ...
  % Upon manually examining the OpenIE systems and their evaluation, we figured out an even deeper rooted problem. The benchmarks used to evaluate these systems were themselves flawed. Wire57 used a tiny gold dataset of only 57 sentences while RelVis had non-intuitive evaluation policies. OIE2016, which was the state-of-the-art OpenIE benchmark at that time, also had a noisy gold dataset (refer to table \ref{tab:gold_example}) as well as non-intuitive evaluation schemes.
...
\section{Contributions}
  ...
  \begin{itemize}
    ...
    \todo{add contributions from imojie and MLIL}
    \item % IMoJIE 
    \item % MLIL
  \end{itemize}
